...
이 로드맵에서 Week는 일정과 목표를 설명하는 단위이고, Phase는 `Feature_Work_Plan_KR.md`의 구현 범위를 설명하는 단위다.
...
UE 5.4에서 빌드 가능한 Editor Plugin 골격을 만들고, 에디터 창 안에서 선택 Asset 기준 Dependencies Depth 1을 Tree View에 표시한다.
...
- `BP_Dummy -> M_Dummy` 같은 직접 Dependency를 확인할 수 있다.

## 2주차: 플러그인이 “도구 구조를 갖췄다”는 증명

### 목표

Dependencies / Referencers 전환, Max Depth, 필터, Content Browser Sync를 추가해 실제 분석 도구의 기본 구조를 갖춘다.
