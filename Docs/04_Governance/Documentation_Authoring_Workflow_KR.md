...
7. `LocalNotes/tools/validate_github_public_docs.ps1`가 로컬 작업장에 있으면 자동 검증 가능한 범위를 추가로 검사한다.
...
4. 로컬 작업장에 `LocalNotes/tools/validate_github_public_docs.ps1`가 있으면 자동 검증 가능한 범위를 확인한다.
