...
`Metadata`는 `LocalNotes` 전용 작업 관리 정보다. `Docs`로 승격하거나 GitHub Issue / PR 본문으로 제출할 때는 제거한다.
...
`LocalNotes/github-issues`의 초안을 GitHub에 올릴 때는 로컬 Metadata를 제거한 제출용 본문을 사용한다. GitHub Issue는 생성된 순간부터 추적 카드이므로 본문에 `Draft` 상태를 남기지 않는다.
...
PR 본문도 Issue와 동일하게 로컬 Metadata를 제거한 제출용 본문을 사용한다. PR이 Draft 상태인지 여부는 GitHub PR 상태로 표현하고, 본문에 `Status: Draft`를 적지 않는다.
