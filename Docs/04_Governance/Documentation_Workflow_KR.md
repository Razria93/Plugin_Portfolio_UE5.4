...
GitHub Architecture / Verification Issue, PR, PR 댓글, 상위 Plan Issue 댓글의 작성/게시 작업장은 `Docs`가 아니라 `LocalNotes/github`에 둔다.
...
GitHub Issue public 본문은 카테고리별로 섹션 순서를 고정한다. 과거에 이미 게시된 Issue는 원문 보존을 우선하지만, 새로 생성하거나 활성 Issue를 수정할 때는 아래 형식을 따른다.

`Summary`는 메타데이터가 아니라 독자가 첫 5초 안에 목적을 이해하기 위한 요약이다. Issue의 `Summary`는 1문단 1~2문장으로 작성하고, bullet을 사용하지 않는다. 문체는 존댓말이 아니라 `정리한다`, `기록한다`, `검증한다` 같은 평서형으로 통일한다.
...
위 정보는 `Scope`, `Build Verification`, `Editor UI Verification`, `Screenshots`, `Documentation`, `Related PR` 같은 전용 섹션에 둔다.
...
Verification Issue는 검증 스크린샷이 없는 경우에도 `Screenshots` 섹션을 둔다. 이미지가 없으면 `없음` 또는 `이번 검증에서는 별도 이미지 없음`처럼 명시한다.
...
Plan Issue 댓글은 다음 섹션 순서를 표준으로 둔다.
...
feature 완료 로그 댓글은 검증 내용이 단순하더라도 `검증` 섹션을 생략하지 않는다. 빌드만 확인했으면 빌드 결과를 쓰고, 직접 확인하지 못한 항목은 `남은 제한`에 남긴다.
...
- 새 `[Plan]` Issue는 반복 생성하지 않는다. 이미 존재하는 상위 Plan Issue에는 `work plan`을 유지한다.
...
PR의 `Summary`는 3~5개 bullet을 기본으로 한다. 각 bullet은 이번 PR에서 바뀐 핵심 변경을 짧게 적는다. `했습니다` 같은 존댓말 문장보다 `추가`, `분리`, `정리`, `검증` 같은 명사형 또는 짧은 평서형을 우선한다.

PR `Summary`에는 PR 번호, Issue 번호, 브랜치명, 상세 검증 로그, 스크린샷 경로를 넣지 않는다. 해당 정보는 `Verification`, `Screenshots`, `Documentation`, `Related Issues`에 둔다.
