...
- PR 본문에 포함한 이미지는 GitHub에서 렌더링되는 repo 직접 주소로 작성되어 있다.
...
- GitHub PR / Issue / 댓글로 게시될 draft와 public 본문은 모두 GitHub repo 이미지 직접 주소를 사용한다.
- PR 작성/검토 중에는 `<git-branch>` 기준 직접 주소를 사용하고, merge 이후 장기 보존/보강 댓글에는 `main` 기준 직접 주소를 사용한다.
- 이미 merge된 과거 PR은 본문을 재작성하지 않고 댓글로 GitHub repo 이미지 직접 주소를 남긴다.
- `Docs` 내부 검증 로그에만 상대 경로를 사용한다.
