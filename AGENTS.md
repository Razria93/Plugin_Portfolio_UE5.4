...
GitHub Issue 라벨은 `Docs/04_Governance/Documentation_Workflow_KR.md` 기준을 따릅니다. `[Architecture]`는 `system architecture`, `[Verification]`은 `verification log`를 사용합니다. 기존 상위 `[Plan]` Issue에는 `work plan`을 유지하되, 브랜치마다 새 Plan Issue 본문은 만들지 않습니다.

GitHub Architecture / Verification Issue, PR 본문, Plan Issue 누적 댓글, PR 스크린샷/댓글은 `LocalNotes/github/draft`에서 초안을 작성한 뒤 `LocalNotes/github/public` 제출용 본문으로 변환합니다. `gh` CLI 업로드는 `LocalNotes/github/public` 파일만 사용합니다.

GitHub 게시 전에는 같은 카테고리의 GitHub 공개 기록 형식을 확인합니다. 게시 후 GitHub 본문을 수정했다면 대응되는 `LocalNotes/github/public` 파일도 동일하게 갱신합니다. 상세 기준은 `Docs/04_Governance/Documentation_Workflow_KR.md`를 따릅니다.
...
상위 `[Plan]` Issue 댓글은 PR 스크린샷 댓글 템플릿을 재사용하지 않습니다. 첫 진행 댓글 갱신에는 `LocalNotes/templates/github_draft_plan_progress_summary_comment_template_KR.md`, feature 완료 로그 댓글에는 `LocalNotes/templates/github_draft_plan_feature_comment_template_KR.md`를 사용합니다.

GitHub Issue public 본문은 카테고리별 표준 섹션을 유지합니다. Architecture Issue는 `Summary / Scope / Design Notes / Documentation / Related PR`, Verification Issue는 `Summary / Build Verification / Editor UI Verification / Screenshots / Unverified / Documentation / Related PR`, Plan feature 댓글은 `완료 내용 / 검증 / 남은 제한 / Related PR` 순서를 따릅니다.

GitHub Issue의 `Summary`에는 branch, PR 번호, issue 번호, 스크린샷 경로, 문서 링크 목록 같은 메타정보를 넣지 않습니다. Issue Summary는 1문단 1~2문장 평서형으로 쓰고, PR Summary는 3~5개 bullet로 핵심 변경만 적습니다. 존댓말 문장은 GitHub 게시 본문에서 사용하지 않습니다.
...
검증 스크린샷이 feature 완료 증거로 필요하면 최종 이미지를 `Docs/03_Verification/Screenshots/<branch-name>/`로 승격하고 PR 또는 검증 로그에서 링크합니다. GitHub PR / Issue / 댓글의 draft와 public 본문에는 repo 이미지 직접 주소를 사용합니다. PR 작성 중에는 브랜치 기준 URL, merge 이후 장기 보존/보강 댓글에는 `main` 기준 URL을 사용합니다. Docs 내부 검증 로그에는 상대 경로를 사용합니다. 임시 이미지는 `LocalNotes/screenshots`에 둡니다.

`LocalNotes` Markdown은 번호 없는 `<topic>_<document_type>_KR.md` 파일명을 사용합니다. GitHub Issue 번호, PR 번호, Phase 번호는 파일명이 아니라 `draft` Metadata 또는 `public` Summary에 기록합니다.

`LocalNotes/github/draft`의 `Metadata`와 `Draft` 섹션은 로컬 초안 전용입니다. `Docs` 승격본이나 `LocalNotes/github/public` 및 GitHub 게시 본문에는 포함하지 않습니다.
